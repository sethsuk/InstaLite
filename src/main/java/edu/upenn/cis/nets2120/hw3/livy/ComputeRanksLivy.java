...
public class ComputeRanksLivy {
    ...
    public static void main(String[] args)
        ...
        // Below is a call to Apache Livy to run a SocialRankJob with backlinks = true.
        // This part is already done for you.
        System.out.println("Starting social rank calculation");
        
        SocialRankJob blJob = new SocialRankJob(d_max, i_max, 1000, true, debug);
        
        List<String> blItems = new ArrayList<>();
        ...
        List<String> noBlItems = new ArrayList<>();

        List<MyPair<String, Double>> noBacklinksResult = SparkJob.runJob(livy, noBlJob);
        System.out.println("With backlinks: " + noBacklinksResult);
        try (PrintStream out = new PrintStream(new FileOutputStream("socialrank-livy-nobacklinks.csv"))) {
            for (MyPair<String, Double> item : noBacklinksResult) {
                out.println(item.getLeft() + "," + item.getRight());
                noBlItems.add(item.getLeft());
            }
            out.close();
        } catch (Exception e) {
            logger.error("Error writing to file: " + e.getMessage());
        ...
        for (MyPair<String, Double> entry: backlinksResult) {
            if(noBlItems.contains(entry.getLeft())){
                inBoth += entry.getLeft() + ",";
            } else {
                blOnly += entry.getLeft() + ",";
            }
        }

        for (MyPair<String, Double> entry: noBacklinksResult) {
            if(!blItems.contains(entry.getLeft())){
                nBlOnly += entry.getLeft() + ",";
            } 
        }
