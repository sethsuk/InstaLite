...
const fetchReplies = async (parent_id) => {
    const sql = `
        SELECT comments.comment_id, comments.content, u.username, u.pfp_url, date(comments.timestamp) AS timestam, comments.parent_id, GROUP_CONCAT(DISTINCT CONCAT('#', hashtags.tag) ORDER BY hashtags.tag ASC SEPARATOR ', ') AS hashtags
        FROM comments
        LEFT JOIN hashtags_to_comments htc ON comments.comment_id = htc.comment_id
        LEFT JOIN hashtags ON htc.hashtag_id = hashtags.hashtag_id
        LEFT JOIN users u ON u.user_id = comments.user_id
